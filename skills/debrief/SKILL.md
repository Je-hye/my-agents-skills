...
- 세션이 길어져 인사이트와 할 일이 많이 쌓였다고 느낄 때 — **단, 세션을 닫기 직전에만**

## 언제 사용하지 않는가

- **세션 중간** — 인사이트가 쌓여도 중간에 실행하지 않는다. 중간 정리는 `/brief` 사용
- `/brief` 실행 전 — `/brief` → `/debrief` 순서를 지킨다
- **`/brief`와 혼용 금지** — `/brief`는 세션 중간 스냅샷, `/debrief`는 세션 종료 시 **한 번만**
...
**외부 시스템 반영 현황** (세션에서 외부 시스템 쓰기 작업이 있었을 때만 출력):
- Confluence 직접 수정: N건 / 댓글 게시: N건
- GitHub PR: 없음 / 머지 N개 / 생성 N개
- Discord 공지: 완료 / 미완료 / 해당 없음

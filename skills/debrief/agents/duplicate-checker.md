...
automation-scout 출력 태그를 다음과 같이 분류한다:
- `[SKILL]` → "스킬 개선" 섹션의 `[SKILL-NEW]`로 변환
- `[HOOK]` → "자동화 기회" 섹션에 별도 보존 (`[HOOK]` 그대로)
- `[COMMAND]` (automation-scout) → "자동화 기회" 섹션에 보존 (`[HOOK]` 그대로, Notes Commands/ 기록 제안인 `[COMMAND]`와 구분)

자동화 기회가 있으면 출력에 아래 섹션을 추가한다:

## 자동화 기회 (N건)
- [HOOK] ...
- [COMMAND] alias/script 자동화 ...

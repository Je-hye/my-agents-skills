...
핸드오프 문서는 항상 파일로 저장하고 채팅창에도 함께 출력한다.

**기본 저장 경로:** `~/.claude/handoffs/YYYY-MM-DD-HHMM.md`

저장 절차:
```bash
mkdir -p ~/.claude/handoffs
# 파일명에 날짜·시간 포함 (같은 날 여러 번 가능)
```

저장 후 사용자에게 다음을 알린다:
1. 저장된 파일 경로
2. 새 세션에서 사용하는 방법: 첫 메시지에 `@~/.claude/handoffs/YYYY-MM-DD-HHMM.md` 입력

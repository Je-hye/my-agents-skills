...
핸드오프 문서를 작성할 때 아래 7개 섹션을 순서대로 채운다.

### 1. Mission (1문장)
이 세션이 달성하려 한 것. 완료됐으면 "완료됨" 표시.
...
메모리 파일(`~/.claude/projects/*/memory/`)에 관련 제약이 있으면 함께 포함하고 출처를 명시한다.

### 6. Next Steps (순서 있는 체크리스트)
구체적이고 실행 가능한 작업. 막연한 표현("구현하기") 대신 파일명과 행동 포함.

### 7. Open Questions
아직 결정하지 않은 것. 새 에이전트에게 결정을 내리라고 시키지 말고 context만 전달한다.

---

## 출력 형식

핸드오프 문서는 다음 중 하나로 출력한다.

**옵션 A — 직접 출력 (기본값)**
채팅창에 코드 블록으로 출력. 사용자가 복사해서 새 세션 첫 메시지에 붙여넣는다.

**옵션 B — 파일 저장**
`~/.claude/jobs/<session-id>/handoff.md` 또는 사용자가 지정한 경로에 저장.
새 세션에서 `@path/to/handoff.md`로 참조한다.

사용자가 경로를 지정하면 B, 아니면 A로 출력한다.
...
## 컨텍스트가 이미 압축된 경우
세션이 길어져 컨텍스트 압축이 발생했다면, 핸드오프를 작성하기 전에:
1. `~/.claude/projects/*/memory/MEMORY.md`를 확인해 관련 메모리를 Critical Constraints에 포함한다.
...
| 파일 경로 없이 "파일 수정함" | 실제 경로와 변경 내용 기록 |

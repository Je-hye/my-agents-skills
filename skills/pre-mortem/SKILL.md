...
| **Pessimist** ⚑ | Dependency failures, platform shifts, timing, distribution, structural single-points-of-failure, domino effects | Grim satisfaction — told you so, saw it coming |
...
| Role | What they catch | Emotional register |
|---|---|---|
| **Burned Expert** | Pattern-matches to prior failures at similar products — focuses on maintenance burden, operational drag, and long-term costs that looked manageable at launch. **Not technical breakage (Saboteur's domain) — the overlap to avoid.** | Controlled fury — watched a nearly identical thing become a maintenance nightmare, not again |
...
| **Privacy / Trust Prosecutor** | Permissions, cloud processing, billing, file mutations, surveillance vibes, consent theater. For AI/agent products: memory file access scope, what data persists between sessions, what Claude can read/write on behalf of the user | Principled outrage — every ambiguity is treated as a deliberate violation |
| **Maintenance Auditor** | 6-month maintenance cost only: support ticket archaeology, update friction, bus factor, dependency rot, the moment the founder stops having time to fix it. Not launch risk — survival risk. | Grim accountancy — describes the Tuesday six months from now when this becomes a second job |

⚑ **Pessimist is always required.** In practice it most reliably surfaces structural single-points-of-failure that other roles miss entirely (e.g., "this whole thing breaks if /schedule session isn't running").

For small exercises: use 4 core + 2 perspective-expanding roles (Burned Expert + Outsider recommended as defaults).
For launch-critical exercises: use all 6 core + Reviewer/Critic + Privacy Prosecutor + Emotional Witness + Maintenance Auditor.
...
- **Saboteur, Historian** — code-aware subagents (technical breakage, brittle integrations, what the code already warns about)
- **Burned Expert, Maintenance Auditor** — code-aware subagents focused on long-term cost and operational drag (not technical correctness)
- **Customer Advocate, Support Lead, Pessimist, Emotional Witness, Outsider, Critic, Privacy/Trust Prosecutor, Devil's Advocate** — external LLMs or additional subagents with no source access (fresh eyes only)

Note: Keep Saboteur and Burned Expert on separate subagents with distinct mandates to prevent overlap. Burned Expert must be explicitly told: "Your domain is maintenance burden and operational drag — not technical breakage. That's Saboteur's job."
